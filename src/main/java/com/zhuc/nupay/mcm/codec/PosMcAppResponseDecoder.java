 ...
 */
public class PosMcAppResponseDecoder extends CumulativeProtocolDecoder {

    private final Logger log = Logger.getLogger(PosMcAppResponseDecoder.class.getName());
    ...
    @Override
    protected boolean doDecode(IoSession is, IoBuffer ib, ProtocolDecoderOutput out) throws Exception {
        int length = -1;
        byte cmd;
        byte[] data = null;
        if (ib.remaining() >= 4 //检查长度
            ...
            if (ib.remaining() >= length) {              //数据已经齐了.解码.
                ...
                Message message = PosMcMessageFactory.createPosMcMessage(msg, false);
                if (message != null) {
                    log.debug("解码一个数据包,"+message.getClass()+",长度:" + length);
